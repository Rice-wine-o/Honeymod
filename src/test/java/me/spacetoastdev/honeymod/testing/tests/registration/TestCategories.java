...
class TestCategories {
    ...
    void testCategoryGetters() {
        ...
        Assertions.assertEquals(new NamespacedKey(HoneymodPlugin.instance(), "getter_test"), category.getKey());
    ...
    void testHidden() {
        ...
        HoneymodPlugin.getItemCfg().setValue("DISABLED_CATEGORY_ITEM.enabled", false);
    ...
    void testFlexCategory() {
        FlexCategory category = new FlexCategory(new NamespacedKey(plugin, "flex"), new CustomItem(Material.REDSTONE, "&4Weird flex but ok")) {
            ...
            @Override
            public void open(Player p, PlayerProfile profile, HoneymodGuideMode layout) {
                // Nothing
            }

            @Override
            public boolean isVisible(Player p, PlayerProfile profile, HoneymodGuideMode layout) {
                return true;
            }
        };
