...
class TestBackpackListener {
    ...
    private PlayerBackpack openMockBackpack(Player player, String id, int size) throws InterruptedException {
        ...
        HoneymodBackpack honeymodBackpack = new HoneymodBackpack(size, category, item, RecipeType.NULL, new ItemStack[9]);
        honeymodBackpack.register(plugin);

        listener.openBackpack(player, item, honeymodBackpack);
        return backpack;
    }
