...
import me.spacetoastdev.honeymod.testing.annotations.SlimefunItemsSource;
...
class TestCargoNodeListener {
    ...
    void testNonCargoNode() {
        HoneymodPlugin.getRegistry().setBackwardsCompatible(true);
        Player player = server.addPlayer();
        Location l = new Location(player.getWorld(), 190, 50, 400);
        Block b = l.getBlock();
        Block against = b.getRelative(BlockFace.DOWN);
        ...
        listener.onCargoNodePlace(event);
        Assertions.assertFalse(event.isCancelled());
        HoneymodPlugin.getRegistry().setBackwardsCompatible(false);
    }
