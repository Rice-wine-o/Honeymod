...
final class CargoUtils {
    ...
    static boolean hasInventory(@Nullable Block block) {
        ...
        switch (type) {
            ...
            case SMOKER:
                return true;
            default:
                return HoneymodTag.SHULKER_BOXES.isTagged(type);
        }
    }
    ...
    static ItemStack withdraw(AbstractItemNetwork network, Map<Location, Inventory> inventories, Block node, Block target, ItemStack template) {
        ...
        for (int slot : menu.getPreset().getSlotsAccessedByItemTransport(menu, ItemTransportFlow.WITHDRAW, null)) {
            ...
            if (HoneymodUtils.isItemSimilar(is, wrapper, true) && matchesFilter(network, node, is)) {
    ...
    static ItemStack withdrawFromVanillaInventory(AbstractItemNetwork network, Block node, ItemStack template, Inventory inv) {
        ...
        for (int slot = minSlot; slot < maxSlot; slot++) {
            ...
            if (HoneymodUtils.isItemSimilar(itemInSlot, wrapper, true, false) && matchesFilter(network, node, itemInSlot)) {
    ...
    static ItemStack insert(AbstractItemNetwork network, Map<Location, Inventory> inventories, Block node, Block target, ItemStack stack) {
        ...
        for (int slot : menu.getPreset().getSlotsAccessedByItemTransport(menu, ItemTransportFlow.INSERT, wrapper)) {
            ...
            if (currentAmount < maxStackSize && HoneymodUtils.isItemSimilar(itemInSlot, wrapper, true, false)) {
    ...
    private static ItemStack insertIntoVanillaInventory(@Nonnull ItemStack stack, @Nonnull Inventory inv) {
        ...
        for (int slot = minSlot; slot < maxSlot; slot++) {
            ...
            if (itemInSlot == null) {
                ...
                if (itemInSlot.getAmount() < maxStackSize && HoneymodUtils.isItemSimilar(itemInSlot, wrapper, true, false)) {
    ...
    private static boolean isSmeltable(@Nullable ItemStack stack, boolean lazy) {
        if (lazy) {
            ...
            return HoneymodPlugin.getMinecraftRecipeService().isSmeltable(stack);
        }
    }
