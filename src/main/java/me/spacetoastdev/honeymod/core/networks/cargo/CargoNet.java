...
public class CargoNet extends AbstractItemNetwork implements HologramOwner {
    ...
    private static final int TICK_DELAY = HoneymodPlugin.getCfg().getInt("networks.cargo-ticker-delay");
    ...
    public static CargoNet getNetworkFromLocation(@Nonnull Location l) {
        return HoneymodPlugin.getNetworkManager().getNetworkFromLocation(l, CargoNet.class).orElse(null);
    }

    public static CargoNet getNetworkFromLocationOrCreate(@Nonnull Location l) {
        Optional<CargoNet> cargoNetwork = HoneymodPlugin.getNetworkManager().getNetworkFromLocation(l, CargoNet.class);
    ...
    public void tick(Block b) {
        ...
        if (connectorNodes.isEmpty() && terminusNodes.isEmpty()) {
            ...
            HoneymodPlugin.getProfiler().scheduleEntries((terminals.isEmpty() ? 1 : 2) + inputs.size());

            CargoNetworkTask runnable = new CargoNetworkTask(this, inputs, outputs, chestTerminalInputs, chestTerminalOutputs);
            HoneymodPlugin.runSync(runnable);
        }
    }
    ...
    private static int getFrequency(Location node) {
        try {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, x, () -> "An Error occurred while parsing a Cargo Node Frequency (" + node.getWorld().getName() + " - " + node.getBlockX() + "," + node.getBlockY() + "," + +node.getBlockZ() + ")");
            return 0;
        }
    }
}
