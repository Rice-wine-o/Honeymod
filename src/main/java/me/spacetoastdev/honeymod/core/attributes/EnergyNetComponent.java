...
public interface EnergyNetComponent extends ItemAttribute {
            ...
            if (capacity > 0) {
                ...
                if (charge != getCharge(l)) {
                    ...
                    if (getEnergyComponentType() == EnergyNetComponentType.CAPACITOR) {
                        HoneymodUtils.updateCapacitorTexture(l, charge, capacity);
                    }
                }
            }
        } catch (Exception | LinkageError x) {
            HoneymodPlugin.logger().log(Level.SEVERE, x, () -> "Exception while trying to set the energy-charge for \"" + getId() + "\" at " + new BlockPosition(l));
        }
    }
    ...
    default void addCharge(@Nonnull Location l, int charge) {
        ...
        try {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, x, () -> "Exception while trying to add an energy-charge for \"" + getId() + "\" at " + new BlockPosition(l));
        }
    }
    ...
    default void removeCharge(@Nonnull Location l, int charge) {
        ...
        try {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, x, () -> "Exception while trying to remove an energy-charge for \"" + getId() + "\" at " + new BlockPosition(l));
        }
    }
