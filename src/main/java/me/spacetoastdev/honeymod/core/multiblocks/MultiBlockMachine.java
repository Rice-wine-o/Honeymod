package me.spacetoastdev.honeymod.core.multiblocks;
...
import me.spacetoastdev.honeymod.core.handlers.MultiBlockInteractionHandler;
...
public abstract class MultiBlockMachine extends SlimefunItem implements NotPlaceable, RecipeDisplayItem {
    ...
    public void postRegister() {
        HoneymodPlugin.getRegistry().getMultiBlocks().add(multiblock);
    }
    ...
    protected MultiBlockInteractionHandler getInteractionHandler() {
        return (p, mb, b) -> {
            if (mb.equals(getMultiBlock())) {
                if (canUse(p, true) && HoneymodPlugin.getProtectionManager().hasPermission(p, b.getLocation(), ProtectableAction.INTERACT_BLOCK)) {
                    onInteract(p, b);
                }
