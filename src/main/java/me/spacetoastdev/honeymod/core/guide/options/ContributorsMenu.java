...
import me.spacetoastdev.honeymod.core.services.github.Contributor;
...
final class ContributorsMenu {
    ...
    public static void open(Player p, int page) {
        ChestMenu menu = new ChestMenu(HoneymodPlugin.getLocalization().getMessage(p, "guide.title.credits"));
        ...
        List<Contributor> contributors = new ArrayList<>(HoneymodPlugin.getGitHubService().getContributors().values());
    ...
    private static ItemStack getContributorHead(Player p, Contributor contributor) {
        ItemStack skull = HoneymodUtils.getCustomHead(contributor.getTexture());
        ...
        for (Map.Entry<String, Integer> entry : contributor.getContributions()) {
            ...
            if (!info.startsWith("&")) {
                ...
                info = HoneymodPlugin.getLocalization().getMessage(p, "guide.credits.roles." + segments[0]);

                if (segments.length == 2) {
                    info += " &7(" + HoneymodPlugin.getLocalization().getMessage(p, "languages." + segments[1]) + ')';
                }
            }

            if (entry.getValue() > 0) {
                String commits = HoneymodPlugin.getLocalization().getMessage(p, "guide.credits." + (entry.getValue() > 1 ? "commits" : "commit"));
        ...
        if (contributor.getProfile() != null) {
            lore.add("");
            lore.add(ChatColors.color("&7\u21E8 &e") + HoneymodPlugin.getLocalization().getMessage(p, "guide.credits.profile-link"));
        }
