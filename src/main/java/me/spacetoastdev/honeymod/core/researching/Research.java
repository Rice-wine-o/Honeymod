package me.spacetoastdev.honeymod.core.researching;
...
import me.spacetoastdev.honeymod.api.events.PlayerPreResearchEvent;
...
public class Research implements Keyed {
    ...
    public boolean isEnabled() {
        return HoneymodPlugin.getRegistry().isResearchingEnabled() && enabled;
    }
    ...
    public String getName(@Nonnull Player p) {
        String localized = HoneymodPlugin.getLocalization().getResearchName(p, key);
    ...
    /**
     * Bind the specified {@link SlimefunItem HoneymodItems} to this {@link Research}.
     * 
     * @param items
     ...
     * a {@link HoneymodGuideImplementation}.
     *
     * @param guide
     *            The {@link HoneymodGuideImplementation} used.
     * @param player
    ...
    public void unlockFromGuide(HoneymodGuideImplementation guide, Player player, PlayerProfile profile, SlimefunItem sfItem, Category category, int page) {
        if (!HoneymodPlugin.getRegistry().getCurrentlyResearchingPlayers().contains(player.getUniqueId())) {
            if (profile.hasUnlocked(this)) {
                ...
                if (!event.isCancelled()) {
                    if (this.canUnlock(player)) {
                        ...
                        HoneymodPlugin.getLocalization().sendMessage(player, "messages.not-enough-xp", true);
                    }
                }
            }
        }
    ...
    public boolean canUnlock(@Nonnull Player p) {
        ...
        boolean creativeResearch = p.getGameMode() == GameMode.CREATIVE && HoneymodPlugin.getRegistry().isFreeCreativeResearchingEnabled();
    ...
    public void register() {
        HoneymodPlugin.getResearchCfg().setDefaultValue("enable-researching", true);
        String path = key.getNamespace() + '.' + key.getKey();

        if (HoneymodPlugin.getResearchCfg().contains(path + ".enabled") && !HoneymodPlugin.getResearchCfg().getBoolean(path + ".enabled")) {
        ...
        HoneymodPlugin.getResearchCfg().setDefaultValue(path + ".cost", getCost());
        HoneymodPlugin.getResearchCfg().setDefaultValue(path + ".enabled", true);

        setCost(HoneymodPlugin.getResearchCfg().getInt(path + ".cost"));
        enabled = true;

        HoneymodPlugin.getRegistry().getResearches().add(this);
    }
