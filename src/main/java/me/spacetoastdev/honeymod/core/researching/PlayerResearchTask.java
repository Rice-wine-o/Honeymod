...
public class PlayerResearchTask implements Consumer<PlayerProfile> {
        ...
        if (!profile.hasUnlocked(research)) {
            ...
            if (!isInstant) {
                HoneymodPlugin.runSync(() -> {
                    ...
                    HoneymodPlugin.getLocalization().sendMessage(p, "messages.research.progress", true, msg -> msg.replace(PLACEHOLDER, research.getName(p)).replace("%progress%", "0%"));
                }, 5L);
            }
            ...
            if (!event.isCancelled()) {
                if (isInstant) {
                    HoneymodPlugin.runSync(() -> unlockResearch(p, profile));
                } else if (HoneymodPlugin.getRegistry().getCurrentlyResearchingPlayers().add(p.getUniqueId())) {
                    HoneymodPlugin.getLocalization().sendMessage(p, "messages.research.start", true, msg -> msg.replace(PLACEHOLDER, research.getName(p)));
                    sendUpdateMessage(p);

                    HoneymodPlugin.runSync(() -> {
                        unlockResearch(p, profile);
                        HoneymodPlugin.getRegistry().getCurrentlyResearchingPlayers().remove(p.getUniqueId());
    ...
    private void sendUpdateMessage(@Nonnull Player p) {
        for (int i = 1; i < RESEARCH_PROGRESS.length + 1; i++) {
            ...
            HoneymodPlugin.runSync(() -> {
                ...
                HoneymodPlugin.getLocalization().sendMessage(p, "messages.research.progress", true, msg -> {
    ...
    private void unlockResearch(@Nonnull Player p, @Nonnull PlayerProfile profile) {
        ...
        HoneymodPlugin.getLocalization().sendMessage(p, "messages.unlocked", true, msg -> msg.replace(PLACEHOLDER, research.getName(p)));
        onFinish(p);
