...
class GitHubTask implements Runnable {
    ...
    private void grabTextures() {
        ...
        if (requests >= MAX_REQUESTS_PER_MINUTE && HoneymodPlugin.instance() != null && HoneymodPlugin.instance().isEnabled()) {
            // Slow down API requests and wait a minute after more than x requests were made
            Bukkit.getScheduler().runTaskLaterAsynchronously(HoneymodPlugin.instance(), this::grabTextures, 2 * 60 * 20L);
        }
    ...
    private int requestTexture(@Nonnull Contributor contributor, @Nonnull Map<String, String> skins) {
        if (!contributor.hasTexture()) {
            try {
                ...
                // Too many requests
                HoneymodPlugin.logger().log(Level.WARNING, "Attempted to connect to mojang.com, got this response: {0}: {1}", new Object[] { x.getClass().getSimpleName(), x.getMessage() });
                HoneymodPlugin.logger().log(Level.WARNING, "This usually means mojang.com is temporarily down or started to rate-limit this connection, this is not an error message!");

                // Retry after 5 minutes if it was rate-limiting
                if (x.getMessage().contains("429")) {
                    Bukkit.getScheduler().runTaskLaterAsynchronously(HoneymodPlugin.instance(), this::grabTextures, 5 * 60 * 20L);
                }

                return -1;
            } catch (TooManyRequestsException x) {
                HoneymodPlugin.logger().log(Level.WARNING, "Received a rate-limit from mojang.com, retrying in 4 minutes");
                Bukkit.getScheduler().runTaskLaterAsynchronously(HoneymodPlugin.instance(), this::grabTextures, 4 * 60 * 20L);
