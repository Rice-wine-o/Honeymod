...
public class LocalizationService extends SlimefunLocalization {
    ...
    public LocalizationService(@Nonnull HoneymodPlugin plugin, @Nullable String prefix, @Nullable String serverDefaultLanguage) {
        super(plugin);
        ...
        if (serverDefaultLanguage != null) {
            translationsEnabled = HoneymodPlugin.getCfg().getBoolean("options.enable-translations");
            ...
            HoneymodPlugin.logger().log(Level.INFO, "Available languages: {0}", String.join(", ", languages.keySet()));
            save();
        } else {
    ...
    private void setLanguage(@Nonnull String language, boolean reset) {
        ...
        HoneymodPlugin.logger().log(Level.INFO, "Loaded language \"{0}\"", language);
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(plugin.getClass().getResourceAsStream(path), StandardCharsets.UTF_8))) {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, e, () -> "Failed to load language file: \"" + path + "\"");
        }
    ...
    private FileConfiguration streamConfigFile(@Nonnull String file, @Nullable FileConfiguration defaults) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(plugin.getClass().getResourceAsStream(path), StandardCharsets.UTF_8))) {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, e, () -> "Failed to load language file into memory: \"" + path + "\"");
            return null;
        }
    }
}
