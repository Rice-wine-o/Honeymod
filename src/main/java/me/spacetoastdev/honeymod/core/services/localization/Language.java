...
public final class Language {
    ...
    public Language(@Nonnull String id, @Nonnull String hash) {
        ...
        this.id = id;
        this.item = HoneymodUtils.getCustomHead(hash);

        HoneymodPlugin.getItemTextureService().setTexture(item, "_UI_LANGUAGE_" + id.toUpperCase(Locale.ROOT));
    }
    ...
    public double getTranslationProgress() {
        if (id.equals("en")) {
            ...
            if (progress < 0) {
                progress = HoneymodPlugin.getLocalization().calculateProgress(this);
            }
    ...
    public String getName(@Nonnull Player p) {
        String name = HoneymodPlugin.getLocalization().getMessage(p, "languages." + id);
    ...
    public boolean isDefault() {
        return this == HoneymodPlugin.getLocalization().getDefaultLanguage();
    }
