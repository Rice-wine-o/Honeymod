...
abstract class GitHubConnector {
    ...
    void download() {
        ...
        if (github.isLoggingEnabled()) {
            HoneymodPlugin.logger().log(Level.INFO, "Retrieving {0}.json from GitHub...", getFileName());
        }
        ...
        try {
            ...
            if (response.isSuccess()) {
                ...
                if (github.isLoggingEnabled()) {
                    HoneymodPlugin.logger().log(Level.WARNING, "Failed to fetch {0}: {1} - {2}", new Object[] { url, response.getStatus(), response.getBody() });
                }
            ...
            if (github.isLoggingEnabled()) {
                HoneymodPlugin.logger().log(Level.WARNING, "Could not connect to GitHub in time.", e);
            }
    ...
    private JsonNode readCacheFile() {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8))) {
            ...
            HoneymodPlugin.logger().log(Level.WARNING, "Failed to read Github cache file: {0} - {1}: {2}", new Object[] { file.getName(), e.getClass().getSimpleName(), e.getMessage() });
            return null;
        }
    }
    ...
    private void writeCacheFile(@Nonnull JsonNode node) {
        try (FileOutputStream output = new FileOutputStream(file)) {
            ...
            HoneymodPlugin.logger().log(Level.WARNING, "Failed to populate GitHub cache: {0} - {1}", new Object[] { e.getClass().getSimpleName(), e.getMessage() });
        }
    }
}
