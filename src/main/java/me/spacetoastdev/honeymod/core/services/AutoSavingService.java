...
public class AutoSavingService {
     ...
     */
    public void start(@Nonnull HoneymodPlugin plugin, int interval) {
    ...
    private void saveAllPlayers() {
        ...
        if (players > 0) {
            HoneymodPlugin.logger().log(Level.INFO, "Auto-saved all player data for {0} player(s)!", players);
        }
    }
    ...
    private void saveAllBlocks() {
        ...
        if (!worlds.isEmpty()) {
            HoneymodPlugin.logger().log(Level.INFO, "Auto-saving block data... (Next auto-save: {0}m)", interval);
