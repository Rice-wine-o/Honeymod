...
public final class PostSetup {
    ...
    public static void setupWiki() {
        HoneymodPlugin.logger().log(Level.INFO, "Loading Wiki pages...");
        JsonParser parser = new JsonParser();

        try (BufferedReader reader = new BufferedReader(new InputStreamReader(HoneymodPlugin.class.getResourceAsStream("/wiki.json"), StandardCharsets.UTF_8))) {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, "Failed to load wiki.json file", e);
        }
    }

    public static void loadItems() {
        Iterator<SlimefunItem> iterator = HoneymodPlugin.getRegistry().getEnabledSlimefunItems().iterator();

        while (iterator.hasNext()) {
            SlimefunItem item = iterator.next();

            if (item == null) {
                HoneymodPlugin.logger().log(Level.WARNING, "Removed bugged Item ('NULL?')");
                iterator.remove();
            } else {
                try {
                    item.load();
        ...
        HoneymodPlugin.getItemCfg().save();
        HoneymodPlugin.getResearchCfg().save();
        HoneymodPlugin.getRegistry().setAutoLoadingMode(true);
    }

    /**
     * This method counts the amount of {@link SlimefunItem HoneymodItems} registered
     * by Slimefun itself and not by any addons.
     * 
     * @return The amount of {@link SlimefunItem HoneymodItems} added by Slimefun itself
     */
    private static int countNonAddonItems() {
        // @formatter:off
        return (int) HoneymodPlugin.getRegistry().getEnabledSlimefunItems().stream()
                        .filter(item -> item.getAddon() instanceof HoneymodPlugin)
                        .count();
        // @formatter:on
    }

    private static void loadAutomaticCraftingChamber() {
        AutomatedCraftingChamber crafter = (AutomatedCraftingChamber) HoneymodItems.AUTOMATED_CRAFTING_CHAMBER.getItem();
    ...
    private static void loadOreGrinderRecipes() {
        ...
        GrindStone grinder = (GrindStone) HoneymodItems.GRIND_STONE.getItem();
        if (grinder != null) {
        ...
        OreCrusher crusher = (OreCrusher) HoneymodItems.ORE_CRUSHER.getItem();
        if (crusher != null) {
            ItemStack[] input = null;
        ...
        if (!HoneymodPlugin.getCfg().getBoolean("options.legacy-ore-grinder")) {
    ...
    private static void loadSmelteryRecipes() {
        Smeltery smeltery = (Smeltery) HoneymodItems.SMELTERY.getItem();

        if (smeltery != null && !smeltery.isDisabled()) {
            MakeshiftSmeltery makeshiftSmeltery = ((MakeshiftSmeltery) HoneymodItems.MAKESHIFT_SMELTERY.getItem());
            ItemStack[] input = null;
            ...
            for (SlimefunItem item : HoneymodPlugin.getRegistry().getEnabledSlimefunItems()) {
