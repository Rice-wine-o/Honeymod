...
import me.spacetoastdev.honeymod.api.events.AndroidMineEvent;
...
public class MinerAndroid extends ProgrammableAndroid {
    ...
    protected void dig(Block b, BlockMenu menu, Block block) {
        ...
        if (!HoneymodTag.UNBREAKABLE_MATERIALS.isTagged(block.getType()) && !drops.isEmpty()) {
            OfflinePlayer owner = Bukkit.getOfflinePlayer(UUID.fromString(BlockStorage.getLocationInfo(b.getLocation(), "owner")));

            if (HoneymodPlugin.getProtectionManager().hasPermission(owner, block.getLocation(), ProtectableAction.BREAK_BLOCK)) {
