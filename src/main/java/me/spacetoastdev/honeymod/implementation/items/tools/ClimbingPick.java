...
import me.spacetoastdev.honeymod.implementation.settings.ClimbableSurface;
 ...
 *
 */
public class ClimbingPick extends SimpleHoneymodItem<ItemUseHandler> implements DamageableItem, RecipeDisplayItem {
    ...
    protected void addDefaultSurfaces() {
        ...
        for (Material surface : HoneymodTag.CLIMBING_PICK_STRONG_SURFACES.getValues()) {
        ...
        for (Material surface : HoneymodTag.CLIMBING_PICK_WEAK_SURFACES.getValues()) {
    ...
    public ItemUseHandler getItemHandler() {
        return e -> {
            ...
            if (isDualWieldingEnabled() && !isItem(getOtherHandItem(p, e.getHand()))) {
                HoneymodPlugin.getLocalization().sendMessage(p, "messages.climbing-pick.dual-wielding");
                return;
            }
    ...
    private void climb(Player p, EquipmentSlot hand, ItemStack item, Block block) {
        ...
        if (power > 0.05) {
            ...
            if (users.add(p.getUniqueId())) {
                HoneymodPlugin.runSync(() -> users.remove(p.getUniqueId()), COOLDOWN);
            ...
            HoneymodPlugin.getLocalization().sendMessage(p, "messages.climbing-pick.wrong-material");
        }
    }
