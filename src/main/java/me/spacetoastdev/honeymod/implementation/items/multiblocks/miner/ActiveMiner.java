...
class ActiveMiner implements Runnable {
    ...
    public void stop(String error) {
        ...
        if (p != null) {
            HoneymodPlugin.getLocalization().sendMessage(p, error);
        }
        ...
        queue.thenRun(() -> {
            try {
                ...
                for (int y = height; y > 0; y--) {
                    ...
                    if (!HoneymodPlugin.getProtectionManager().hasPermission(Bukkit.getOfflinePlayer(owner), b, ProtectableAction.BREAK_BLOCK)) {
                ...
                HoneymodPlugin.logger().log(Level.SEVERE, e, () -> "An Error occurred while running an Industrial Miner at " + new BlockPosition(chest));
                stop();
            }
        });
    ...
    private void nextColumn() {
        if (x < end.getX()) {
            ...
            if (p != null) {
                ...
                HoneymodPlugin.getLocalization().sendMessage(p, "machines.INDUSTRIAL_MINER.finished", msg -> msg.replace("%ores%", String.valueOf(ores)));
            }

            return;
        }

        HoneymodPlugin.runSync(this, 5);
    }
    ...
    private void setPistonState(Block block, boolean extended) {
        ...
        try {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, e, () -> "An Error occurred while moving a Piston for an Industrial Miner at " + new BlockPosition(block));
            stop();
        }
    }
