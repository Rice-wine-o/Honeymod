...
public class GEOMiner extends AContainer implements RecipeDisplayItem, HologramOwner {
    ...
    public List<ItemStack> getDisplayRecipes() {
        ...
        for (GEOResource resource : HoneymodPlugin.getRegistry().getGEOResources().values()) {
    ...
    private void start(Block b, BlockMenu inv) {
        for (GEOResource resource : HoneymodPlugin.getRegistry().getGEOResources().values()) {
            if (resource.isObtainableFromGEOMiner()) {
                OptionalInt optional = HoneymodPlugin.getGPSNetwork().getResourceManager().getSupplies(resource, b.getWorld(), b.getX() >> 4, b.getZ() >> 4);
                ...
                if (supplies > 0) {
                    ...
                    HoneymodPlugin.getGPSNetwork().getResourceManager().setSupplies(resource, b.getWorld(), b.getX() >> 4, b.getZ() >> 4, supplies - 1);
