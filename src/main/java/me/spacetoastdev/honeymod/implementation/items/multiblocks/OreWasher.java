...
public class OreWasher extends MultiBlockMachine {
    ...
    public OreWasher(Category category, SlimefunItemStack item) {
        ...
        legacyMode = HoneymodPlugin.getCfg().getBoolean("options.legacy-ore-washer");
        dusts = new ItemStack[] { HoneymodItems.IRON_DUST, HoneymodItems.GOLD_DUST, HoneymodItems.COPPER_DUST, HoneymodItems.TIN_DUST, HoneymodItems.ZINC_DUST, HoneymodItems.ALUMINUM_DUST, HoneymodItems.MAGNESIUM_DUST, HoneymodItems.LEAD_DUST, HoneymodItems.SILVER_DUST };
    }
    ...
    protected void registerDefaultRecipes(List<ItemStack> recipes) {
        ...
        recipes.add(HoneymodItems.SIFTED_ORE);
        recipes.add(HoneymodItems.IRON_DUST);
    ...
    public void onInteract(Player p, Block b) {
        ...
        if (state instanceof Dispenser) {
            ...
            for (ItemStack input : inv.getContents()) {
                if (input != null) {
                    if (HoneymodUtils.isItemSimilar(input, HoneymodItems.SIFTED_ORE, true)) {
                        ...
                        if (!legacyMode) {
                            ...
                            // so it really doesn't matter what item the ItemStack is made by. HoneymodItems.DEBUG_FISH
                            // however, signals that it's
                            // not supposed to be given to the player.
                            ItemStack dummyAdding = HoneymodItems.DEBUG_FISH;
                        ...
                        if (outputInv != null) {
                            outputInv.addItem(HoneymodItems.STONE_CHUNK);
                        }

                        return;
                    } else if (HoneymodUtils.isItemSimilar(input, new ItemStack(Material.SAND, 2), false)) {
                        ItemStack output = HoneymodItems.SALT;
                        Inventory outputInv = findOutputInventory(output, dispBlock, inv);

                        removeItem(p, b, inv, outputInv, input, output, 2);

                        return;
                    } else if (HoneymodUtils.isItemSimilar(input, HoneymodItems.PULVERIZED_ORE, true)) {
                        ItemStack output = HoneymodItems.PURE_ORE_CLUSTER;
                        Inventory outputInv = findOutputInventory(output, dispBlock, inv);
