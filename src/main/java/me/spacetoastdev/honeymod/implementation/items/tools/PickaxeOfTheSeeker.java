 ...
 *
 */
public class PickaxeOfTheSeeker extends SimpleHoneymodItem<ItemUseHandler> implements DamageableItem {
    ...
    public ItemUseHandler getItemHandler() {
        return e -> {
            ...
            if (closest == null) {
                HoneymodPlugin.getLocalization().sendMessage(p, "miner.no-ores");
            } else {
        ...
        for (int x = -range; x <= range; x++) {
            for (int y = -range; y <= range; y++) {
                for (int z = -range; z <= range; z++) {
                    ...
                    if (HoneymodTag.PICKAXE_OF_THE_SEEKER_BLOCKS.isTagged(block.getType())) {
