...
public class Crucible extends SimpleHoneymodItem<BlockUseHandler> implements RecipeDisplayItem {
    ...
    private List<ItemStack> getMachineRecipes() {
        ...
        for (Material sapling : HoneymodTag.TERRACOTTA.getValues()) {
    ...
    private boolean craft(Player p, ItemStack input) {
        for (int i = 0; i < recipes.size(); i += 2) {
            ...
            if (HoneymodUtils.isItemSimilar(input, convert, true)) {
    ...
    private void generateLiquid(@Nonnull Block block, boolean water) {
        if (block.getType() == (water ? Material.WATER : Material.LAVA)) {
            ...
            HoneymodPlugin.runSync(() -> placeLiquid(block, water), 50L);
        }
    }
    ...
    private void addLiquidLevel(@Nonnull Block block, boolean water) {
        ...
        if (level == 0) {
            ...
            HoneymodPlugin.runSync(() -> runPostTask(block, water ? Sound.ENTITY_PLAYER_SPLASH : Sound.BLOCK_LAVA_POP, finalLevel), 50L);
        }
    }
    ...
    private void runPostTask(Block block, Sound sound, int times) {
        ...
        if (times < 8) {
            HoneymodPlugin.runSync(() -> runPostTask(block, sound, times + 1), 50L);
        } else {
