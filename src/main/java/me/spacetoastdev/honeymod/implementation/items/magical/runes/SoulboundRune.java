package me.spacetoastdev.honeymod.implementation.items.magical.runes;
 ...
 *
 */
public class SoulboundRune extends SimpleHoneymodItem<ItemDropHandler> {
    ...
    public ItemDropHandler getItemHandler() {
        return (e, p, item) -> {
            if (isItem(item.getItemStack())) {
                ...
                HoneymodPlugin.runSync(() -> activate(p, item), 20L);
            ...
            if (itemStack.getAmount() == 1) {
                ...
                HoneymodPlugin.runSync(() -> {
                    ...
                    if (rune.isValid() && item.isValid() && itemStack.getAmount() == 1) {
                        ...
                        HoneymodUtils.setSoulbound(itemStack, true);
                        l.getWorld().dropItemNaturally(l, itemStack);

                        HoneymodPlugin.getLocalization().sendMessage(p, "messages.soulbound-rune.success", true);
                    } else {
                        HoneymodPlugin.getLocalization().sendMessage(p, "messages.soulbound-rune.fail", true);
                    }
                }, 10L);
            } else {
                HoneymodPlugin.getLocalization().sendMessage(p, "messages.soulbound-rune.fail", true);
    ...
    private boolean findCompatibleItem(Entity entity) {
        if (entity instanceof Item) {
            ...
            return !HoneymodUtils.isSoulbound(item.getItemStack()) && !isItem(item.getItemStack());
        }
