...
public class MultiTool extends SlimefunItem implements Rechargeable {
    ...
    protected ItemUseHandler getItemUseHandler() {
        return e -> {
            ...
            if (!p.isSneaking()) {
                ...
                HoneymodPlugin.getLocalization().sendMessage(p, "messages.multi-tool.mode-change", true, msg -> msg.replace("%device%", "Multi Tool").replace("%mode%", ChatColor.stripColor(itemName)));
    ...
    private ToolUseHandler getToolUseHandler() {
        return (e, tool, fortune, drops) -> {
            ...
            HoneymodPlugin.getLocalization().sendMessage(e.getPlayer(), "messages.multi-tool.not-shears");
            e.setCancelled(true);
        };
    }
