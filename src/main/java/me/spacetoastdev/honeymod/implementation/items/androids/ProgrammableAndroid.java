...
public class ProgrammableAndroid extends SlimefunItem implements InventoryBlock, RecipeDisplayItem {
        ...
        new BlockMenuPreset(getId(), "Programmable Android") {
            ...
            public void newInstance(BlockMenu menu, Block b) {
                ...
                menu.addMenuClickHandler(15, (p, slot, item, action) -> {
                    HoneymodPlugin.getLocalization().sendMessage(p, "android.started", true);
                ...
                menu.addMenuClickHandler(17, (p, slot, item, action) -> {
                    ...
                    HoneymodPlugin.getLocalization().sendMessage(p, "android.stopped", true);
                    return false;
                });
    ...
    public void openScript(Player p, Block b, String sourceCode) {
        ChestMenu menu = new ChestMenu(ChatColor.DARK_AQUA + HoneymodPlugin.getLocalization().getMessage(p, "android.scripts.editor"));
        menu.setEmptySlotsClickable(false);

        menu.addItem(0, new CustomItem(Instruction.START.getItem(), HoneymodPlugin.getLocalization().getMessage(p, "android.scripts.instructions.START"), "", "&7\u21E8 &eLeft Click &7to return to the Android's interface"));
        ...
        for (int i = 1; i < script.length; i++) {
            ...
            if (i == script.length - 1) {
                ...
                menu.addItem(slot, new CustomItem(Instruction.REPEAT.getItem(), HoneymodPlugin.getLocalization().getMessage(p, "android.scripts.instructions.REPEAT"), "", "&7\u21E8 &eLeft Click &7to return to the Android's interface"));
                ...
                if (instruction == null) {
                    HoneymodPlugin.instance().getLogger().log(Level.WARNING, "Failed to parse Android instruction: {0}, maybe your server is out of date?", script[i]);
                    return;
                }

                ItemStack stack = instruction.getItem();
                menu.addItem(i, new CustomItem(stack, HoneymodPlugin.getLocalization().getMessage(p, "android.scripts.instructions." + instruction.name()), "", "&7\u21E8 &eLeft Click &7to edit", "&7\u21E8 &eRight Click &7to delete", "&7\u21E8 &eShift + Right Click &7to duplicate"));
                menu.addMenuClickHandler(i, (pl, slot, item, action) -> {
                    ...
                    try {
                        if (action.isShiftClicked()) {
                            if (script.isAuthor(player)) {
                                HoneymodPlugin.getLocalization().sendMessage(player, "android.scripts.rating.own", true);
                            } else if (script.canRate(player)) {
                                script.rate(player, !action.isRightClicked());
                                openScriptDownloader(player, b, page);
                            } else {
                                HoneymodPlugin.getLocalization().sendMessage(player, "android.scripts.rating.already", true);
                            }
                        } else if (!action.isRightClicked()) {
                            script.download();
                        ...
                        HoneymodPlugin.logger().log(Level.SEVERE, "An Exception was thrown when a User tried to download a Script!", x);
                    }
    ...
    private void uploadScript(Player p, Block b, int page) {
        ...
        for (Script script : Script.getUploadedScripts(getAndroidType())) {
            ...
            if (script.getSourceCode().equals(code)) {
                HoneymodPlugin.getLocalization().sendMessage(p, "android.scripts.already-uploaded", true);
                return;
            }
        }

        p.closeInventory();
        HoneymodPlugin.getLocalization().sendMessages(p, "android.scripts.enter-name");
        int id = nextId;

        ChatInput.waitForPlayer(HoneymodPlugin.instance(), p, msg -> {
            Script.upload(p, getAndroidType(), id, msg, code);
            HoneymodPlugin.getLocalization().sendMessages(p, "android.scripts.uploaded");
    ...
    public void openScriptEditor(Player p, Block b) {
        ...
        menu.addMenuClickHandler(1, (pl, slot, item, action) -> {
            if (PatternUtils.DASH.split(BlockStorage.getLocationInfo(b.getLocation()).getString("script")).length <= MAX_SCRIPT_LENGTH) {
                ...
                pl.closeInventory();
                HoneymodPlugin.getLocalization().sendMessage(pl, "android.scripts.too-long");
            }
            return false;
        });
    ...
    protected void editInstruction(Player p, Block b, String[] script, int index) {
        ...
        for (Instruction instruction : getValidScriptInstructions()) {
            menu.addItem(i, new CustomItem(instruction.getItem(), HoneymodPlugin.getLocalization().getMessage(p, "android.scripts.instructions." + instruction.name())), (pl, slot, item, action) -> {
    ...
    private void registerDefaultFuelTypes() {
        switch (getFuelSource()) {
            ...
            case LIQUID:
                ...
                registerFuelType(new MachineFuel(200, HoneymodItems.OIL_BUCKET));
                registerFuelType(new MachineFuel(500, HoneymodItems.FUEL_BUCKET));
                break;
            case NUCLEAR:
                registerFuelType(new MachineFuel(2500, HoneymodItems.URANIUM));
                registerFuelType(new MachineFuel(1200, HoneymodItems.NEPTUNIUM));
                registerFuelType(new MachineFuel(3000, HoneymodItems.BOOSTED_URANIUM));
                break;
            default:
    ...
    protected void tick(Block b, Config data) {
        ...
        if ("false".equals(data.getString("paused"))) {
            ...
            if (fuel < 0.001) {
                ...
                if (instruction == null) {
                    HoneymodPlugin.instance().getLogger().log(Level.WARNING, "Failed to parse Android instruction: {0}, maybe your server is out of date?", script[index]);
                    return;
                }
    ...
    private boolean insertFuel(BlockMenu menu, Inventory dispenser, int slot, ItemStack currentFuel, ItemStack newFuel) {
        if (currentFuel == null) {
            ...
            return true;
        } else if (HoneymodUtils.isItemSimilar(newFuel, currentFuel, true, false)) {
    ...
    protected void move(Block b, BlockFace face, Block block) {
        if (block.getY() > 0 && block.getY() < block.getWorld().getMaxHeight() && block.isEmpty()) {
            ...
            HoneymodPlugin.runSync(() -> SkullBlock.setFromBase64(block, texture));
