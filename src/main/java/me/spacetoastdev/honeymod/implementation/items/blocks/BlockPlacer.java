...
import me.spacetoastdev.honeymod.api.items.settings.MaterialTagSetting;
...
public class BlockPlacer extends SlimefunItem {
    ...
    private final ItemSetting<List<String>> unplaceableBlocks = new MaterialTagSetting("unplaceable-blocks", HoneymodTag.UNBREAKABLE_MATERIALS);
    ...
    private BlockDispenseHandler onBlockDispense() {
        return (e, dispenser, facedBlock, machine) -> {
            ...
            if (HoneymodTag.SHULKER_BOXES.isTagged(material)) {
                /*
            ...
            if (!material.isBlock() || HoneymodTag.BLOCK_PLACER_IGNORED_MATERIALS.isTagged(material)) {
                /*
    ...
    private boolean hasPermission(Dispenser dispenser, Block target) {
        ...
        return HoneymodPlugin.getProtectionManager().hasPermission(player, target, ProtectableAction.PLACE_BLOCK);
    }
