...
public class Composter extends SimpleHoneymodItem<BlockUseHandler> implements RecipeDisplayItem {
            ...
            if (block.isPresent()) {
                ...
                if (p.hasPermission("slimefun.inventory.bypass") || HoneymodPlugin.getProtectionManager().hasPermission(p, b.getLocation(), ProtectableAction.INTERACT_BLOCK)) {
                    ...
                    if (output != null) {
                        ...
                        tasks.execute(HoneymodPlugin.instance());
                    } else {
    ...
    private ItemStack getOutput(Player p, ItemStack input) {
        for (int i = 0; i < recipes.size(); i += 2) {
            ...
            if (convert != null && HoneymodUtils.isItemSimilar(input, convert, true)) {
