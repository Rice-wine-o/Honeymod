 ...
 *
 */
public abstract class AbstractEntityAssembler<T extends Entity> extends SimpleHoneymodItem<BlockTicker> implements EnergyNetComponent {
    ...
    private boolean findResource(BlockMenu menu, ItemStack item, int[] slots) {
        ...
        for (int slot : slots) {
            if (HoneymodUtils.isItemSimilar(menu.getItemInSlot(slot), item, true, false)) {
    ...
    private void consumeResources(BlockMenu inv) {
        ...
        for (int slot : bodySlots) {
            if (HoneymodUtils.isItemSimilar(inv.getItemInSlot(slot), getBody(), true, false)) {
        ...
        for (int slot : headSlots) {
            if (HoneymodUtils.isItemSimilar(inv.getItemInSlot(slot), getHead(), true, false)) {
                int amount = inv.getItemInSlot(slot).getAmount();
