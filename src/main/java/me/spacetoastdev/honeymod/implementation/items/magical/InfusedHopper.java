 ...
 * @see InfusedMagnet
 */
public class InfusedHopper extends SimpleHoneymodItem<BlockTicker> {
    ...
    private boolean isValidItem(@Nonnull Location l, @Nonnull Entity entity) {
        if (entity instanceof Item && entity.isValid()) {
            ...
            return item.getPickupDelay() <= 0 && !HoneymodUtils.hasNoPickupFlag(item) && item.getLocation().distanceSquared(l) > 0.25;
        }
