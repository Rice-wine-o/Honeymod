package me.spacetoastdev.honeymod.implementation.items.elevator;
 ...
 * @author Walshy
 */
public class ElevatorPlate extends SimpleHoneymodItem<BlockUseHandler> {
    ...
    public void openInterface(Player p, Block b) {
        ...
        if (floors.size() < 2) {
            HoneymodPlugin.getLocalization().sendMessage(p, "machines.ELEVATOR.no-destinations", true);
        } else {
    ...
    private void openFloorSelector(Block b, List<ElevatorFloor> floors, Player p, int page) {
        ChestMenu menu = new ChestMenu(HoneymodPlugin.getLocalization().getMessage(p, "machines.ELEVATOR.pick-a-floor"));
        ...
        for (int i = 0; i < Math.min(GUI_SIZE, floors.size() - index); i++) {
            ...
            if (floor.getAltitude() == b.getY()) {
                menu.addItem(i, new CustomItem(
                    ...
                    HoneymodPlugin.getLocalization().getMessage(p, "machines.ELEVATOR.current-floor") + ' ' + ChatColor.WHITE + floor.getName()
                ), ChestMenuUtils.getEmptyClickHandler());
            } else {
                menu.addItem(i, new CustomItem(
                    Material.PAPER,
                    ChatColor.GRAY.toString() + floor.getNumber() + ". " + ChatColor.BLACK + floor.getName(),
                    HoneymodPlugin.getLocalization().getMessage(p, "machines.ELEVATOR.click-to-teleport") + ' ' + ChatColor.WHITE + floor.getName()
    ...
    public void openEditor(Player p, Block b) {
        ...
        menu.addMenuClickHandler(4, (pl, slot, item, action) -> {
            pl.closeInventory();
            pl.sendMessage("");
            HoneymodPlugin.getLocalization().sendMessage(p, "machines.ELEVATOR.enter-name");
            pl.sendMessage("");
            ...
            ChatUtils.awaitInput(pl, message -> {
                ...
                pl.sendMessage("");
                HoneymodPlugin.getLocalization().sendMessage(p, "machines.ELEVATOR.named", msg -> msg.replace("%floor%", message));
                pl.sendMessage("");
