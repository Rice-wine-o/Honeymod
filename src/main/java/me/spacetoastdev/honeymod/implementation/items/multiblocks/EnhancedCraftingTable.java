    ...
    private void craft(Inventory inv, Block dispenser, Player p, Block b, ItemStack output) {
        ...
        if (outputInv != null) {
            ...
            if (sfItem instanceof HoneymodBackpack) {
                upgradeBackpack(p, inv, (HoneymodBackpack) sfItem, output);
            }
    ...
    private boolean isCraftable(Inventory inv, ItemStack[] recipe) {
        for (int j = 0; j < inv.getContents().length; j++) {
            if (!HoneymodUtils.isItemSimilar(inv.getContents()[j], recipe[j], true)) {
                if (SlimefunItem.getByItem(recipe[j]) instanceof HoneymodBackpack) {
                    if (!HoneymodUtils.isItemSimilar(inv.getContents()[j], recipe[j], false)) {
                        return false;
                    }
                } else {
                    return false;
