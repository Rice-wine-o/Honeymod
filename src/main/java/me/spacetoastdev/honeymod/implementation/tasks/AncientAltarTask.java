...
import me.spacetoastdev.honeymod.api.events.AncientAltarCraftEvent;
...
public class AncientAltarTask implements Runnable {
    ...
    private final AncientPedestal pedestalItem = (AncientPedestal) HoneymodItems.ANCIENT_PEDESTAL.getItem();
    ...
    public void run() {
        ...
        this.stage += 1;
        HoneymodPlugin.runSync(this, speed);
    }
    ...
    private void checkPedestal(@Nonnull Block pedestal) {
        ...
        if (!item.isPresent() || positionLock.remove(item.get()) == null) {
            ...
            entity.remove();
            entity.removeMetadata("no_pickup", HoneymodPlugin.instance());
        }
    }
