...
public class TickerTask implements Runnable {
     ...
     */
    public void start(@Nonnull HoneymodPlugin plugin) {
        this.tickRate = HoneymodPlugin.getCfg().getInt("URID.custom-ticker-delay");
    ...
    public void run() {
        try {
            ...
            running = true;
            HoneymodPlugin.getProfiler().start();
            ...
            Iterator<BlockStorage> worlds = HoneymodPlugin.getRegistry().getWorlds().values().iterator();
    ...
    private void tickChunk(ChunkPosition chunk, Set<BlockTicker> tickers, Set<Location> locations) {
        try {
            ...
            HoneymodPlugin.logger().log(Level.SEVERE, x, () -> "An Exception has occurred while trying to resolve Chunk: " + chunk);
        }
    }
        ...
        if (item != null && item.getBlockTicker() != null) {
            try {
                if (item.getBlockTicker().isSynchronized()) {
                    HoneymodPlugin.getProfiler().scheduleEntries(1);
    ...
    private void tickBlock(Location l, Block b, SlimefunItem item, Config data, long timestamp) {
        try {
            ...
            HoneymodPlugin.getProfiler().closeEntry(l, item, timestamp);
        }
    }
    ...
    private void reportErrors(Location l, SlimefunItem item, Throwable x) {
        ...
        if (errors == 1) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(HoneymodPlugin.instance(), () -> l.getBlock().setType(Material.AIR));
        } else {
            bugs.put(position, errors);
        }
    }
