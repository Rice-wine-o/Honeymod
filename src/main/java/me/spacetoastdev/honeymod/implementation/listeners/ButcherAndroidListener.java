...
import me.spacetoastdev.honeymod.implementation.items.androids.AndroidInstance;
...
public class ButcherAndroidListener implements Listener {
    ...
    public ButcherAndroidListener(@Nonnull HoneymodPlugin plugin) {
        ...
        if (e.getEntity().hasMetadata(METADATA_KEY)) {
            ...
            HoneymodPlugin.runSync(() -> {
                ...
                for (Entity n : e.getEntity().getNearbyEntities(0.5D, 0.5D, 0.5D)) {
                    if (n instanceof Item && n.isValid() && !HoneymodUtils.hasNoPickupFlag((Item) n)) {
            ...
            e.getEntity().removeMetadata(METADATA_KEY, HoneymodPlugin.instance());
        }
    }
