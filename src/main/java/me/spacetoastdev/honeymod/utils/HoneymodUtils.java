...
import me.spacetoastdev.honeymod.implementation.tasks.CapacitorTextureUpdateTask;
 ...
 *
 */
public final class HoneymodUtils {
    ...
    public static void markAsNoPickup(@Nonnull Item item, @Nonnull String context) {
        item.setMetadata(NO_PICKUP_METADATA, new FixedMetadataValue(HoneymodPlugin.instance(), context));
    }
    ...
    private static boolean hasSoulboundFlag(@Nullable ItemMeta meta) {
        if (meta != null) {
            ...
            NamespacedKey key = HoneymodPlugin.getRegistry().getSoulboundDataKey();
    ...
    public static boolean isItemSimilar(@Nullable ItemStack item, @Nullable ItemStack sfitem, boolean checkLore, boolean checkAmount) {
        if (item == null) {
            ...
            if (sfitem instanceof SlimefunItemStack) {
                Optional<String> id = HoneymodPlugin.getItemDataService().getItemData(itemMeta);
    ...
    public static void updateCapacitorTexture(@Nonnull Location l, int charge, int capacity) {
        ...
        HoneymodPlugin.runSync(new CapacitorTextureUpdateTask(l, charge, capacity));
    }
