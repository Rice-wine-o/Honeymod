...
/**
 * The {@link TagParser} is responsible for parsing a JSON input into a {@link HoneymodTag}.
 * 
...
public class TagParser implements Keyed {
     ...
     * 
     * @param key
     *            The {@link NamespacedKey} of the resulting {@link HoneymodTag}
     */
    ...
    /**
     * This constructs a new {@link TagParser} for the given {@link HoneymodTag}
     * 
     * @param tag
     *            The {@link HoneymodTag} to parse inputs for
     */
    TagParser(@Nonnull HoneymodTag tag) {
        this(tag.getKey());
    }

    void parse(@Nonnull HoneymodTag tag, @Nonnull BiConsumer<Set<Material>, Set<Tag<Material>>> callback) throws TagMisconfigurationException {
        String path = "/tags/" + tag.getKey().getKey() + ".json";

        try (BufferedReader reader = new BufferedReader(new InputStreamReader(HoneymodPlugin.class.getResourceAsStream(path), StandardCharsets.UTF_8))) {
    ...
    private void parsePrimitiveValue(String value, Set<Material> materials, Set<Tag<Material>> tags, boolean throwException) throws TagMisconfigurationException {
        ...
        } else if (PatternUtils.HONEYMOD_TAG.matcher(value).matches()) {
            // Get a HoneymodTag enum value for the given key
            String keyValue = PatternUtils.COLON.split(value)[1].toUpperCase(Locale.ROOT);
            HoneymodTag tag = HoneymodTag.getTag(keyValue);
