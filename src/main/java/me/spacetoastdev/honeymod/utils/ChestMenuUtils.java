package me.spacetoastdev.honeymod.utils;
...
public final class ChestMenuUtils {
    ...
    public static ItemStack getBackButton(@Nonnull Player p, String... lore) {
        return new CustomItem(BACK_BUTTON, "&7\u21E6 " + HoneymodPlugin.getLocalization().getMessage(p, "guide.back.title"), lore);
    }

    @Nonnull
    public static ItemStack getMenuButton(@Nonnull Player p) {
        return new CustomItem(MENU_BUTTON, ChatColor.YELLOW + HoneymodPlugin.getLocalization().getMessage(p, "guide.title.settings"), "", "&7\u21E8 " + HoneymodPlugin.getLocalization().getMessage(p, "guide.tooltips.open-category"));
    }

    @Nonnull
    public static ItemStack getSearchButton(@Nonnull Player p) {
        return new CustomItem(SEARCH_BUTTON, meta -> {
            meta.setDisplayName(ChatColors.color(HoneymodPlugin.getLocalization().getMessage(p, "guide.search.name")));

            List<String> lore = Arrays.asList("", ChatColor.GRAY + "\u21E8 " + HoneymodPlugin.getLocalization().getMessage(p, "guide.search.tooltip"));
    ...
    public static ItemStack getPreviousButton(@Nonnull Player p, int page, int pages) {
        if (pages == 1 || page == 1) {
            return new CustomItem(PREV_BUTTON_INACTIVE, meta -> {
                meta.setDisplayName(ChatColor.DARK_GRAY + "\u21E6 " + HoneymodPlugin.getLocalization().getMessage(p, "guide.pages.previous"));
                meta.setLore(Arrays.asList("", ChatColor.GRAY + "(" + page + " / " + pages + ")"));
            });
        } else {
            return new CustomItem(PREV_BUTTON_ACTIVE, meta -> {
                meta.setDisplayName(ChatColor.WHITE + "\u21E6 " + HoneymodPlugin.getLocalization().getMessage(p, "guide.pages.previous"));
                meta.setLore(Arrays.asList("", ChatColor.GRAY + "(" + page + " / " + pages + ")"));
            });
        }
    }
    ...
    public static ItemStack getNextButton(@Nonnull Player p, int page, int pages) {
        if (pages == 1 || page == pages) {
            return new CustomItem(NEXT_BUTTON_INACTIVE, meta -> {
                meta.setDisplayName(ChatColor.DARK_GRAY + HoneymodPlugin.getLocalization().getMessage(p, "guide.pages.next") + " \u21E8");
                meta.setLore(Arrays.asList("", ChatColor.GRAY + "(" + page + " / " + pages + ")"));
            });
        } else {
            return new CustomItem(NEXT_BUTTON_ACTIVE, meta -> {
                meta.setDisplayName(ChatColor.WHITE + HoneymodPlugin.getLocalization().getMessage(p, "guide.pages.next") + " \u21E8");
                meta.setLore(Arrays.asList("", ChatColor.GRAY + "(" + page + " / " + pages + ")"));
            });
        }
    }
